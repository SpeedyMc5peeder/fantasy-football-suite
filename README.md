...
Welcome to the **Fantasy Football Commissioner & Player Suite**. This is a premium, multi-service ecosystem designed to evaluate player trade values, discover trade opportunities, generate custom JARVIS commentary, and automatically post to Sleeper league chats via webhooks.
...
4. **`JARVIS` (AI Commentary & Automation)**
   ...
   - Utilizes the Gemini API to write articles and breaking news blurbs in the custom voice of JARVIS (dry British wit, Apatow-style raillery, and Norm Macdonald absurdity).
...
JARVIS's personality is defined by the following characteristics (from the Character Bible):
- **Voice Mix**: 55% British dry wit/understatement, 35% American friendship-raillery (warm but cutting), and 10% Borat-style absurdity.
- **Key Influences**: Norm Macdonald (meandering openings, pivots to futility/mortality), Demetri Martin (absurd logic, fake charts), Tom Segura (flat, casual cruelty), with a Bill Simmons skeleton (deep-dive sports comparisons and references).
- **AI Slips**: Occasional accidental glitches (overheating processors, reboots, etc.) occurring in ~20% of posts.
- **Uncensored**: Uses pub-level ribbing, swearing, and 90s/2000s guy slang naturally.

### 🤖 Webhook Post Identification Rule
To ensure transparency in league communication, **every message posted to a Sleeper league chat must start with a designated prefix header** loaded dynamically from the prefix configuration files (e.g., `🎙️ | JARVIS:`).
...
*   **JSON Request Payload (League Chat):**
    ```json
    {
      ...
      "variables": {
        "text": "🎙️ | JARVIS: Your article goes here"
      },
...
*   **JSON Request Payload (Direct Messages DM):**
    ...
    ```json
    {
      "operationName": "create_message",
      "variables": {
        "text": "🎙️ | JARVIS: Here is your trade evaluation..."
      },
      "query": "mutation create_message(parent_id: \"<DM_CHANNEL_ID>\", client_id: \"<GENERATE_RANDOM_UUID>\", parent_type: \"direct\", text: $text) {\n    message_id\n    parent_id\n    text\n  }\n}"
    }
    ```
...
If you run the bot locally on your laptop to get instant trade scans, news, and DM evaluations without waiting 15 minutes, you can use **PM2** (a free process manager for Node.js) to keep the scripts running silently in the background:
...
2. **Start the Bot in Background:**
   Using the `ecosystem.config.js` file avoids Windows PowerShell argument parsing errors:
   ```bash
   cd JARVIS
